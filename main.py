...
def send_welcome(message):
    ...
    bot.reply_to(message, "Up an running. Commands are: /start, /help, /train, /predict. Send a .bib or .csv files with your bibliography doe the bot to get to know a bit better first.")

@bot.message_handler(commands=['train'])
def train_classifier(message):
    training = Trainer()
    result_msg = "Finished training, accuracy: " + str(training.report["test_accuracy"]) + " %"
    bot.reply_to(message, result_msg)

@bot.message_handler(commands=['predict'])
def predict_sentence(message):
    if not os.path.isfile("./bib_files/trained_data.joblib"):
        bot.reply_to(message, "The /train_classifier command must be executed before this one.")
    else:
